...
export const PricingSection = () => {
  ...
  return (
    <section id="pricing" className="py-16 sm:py-20 px-4 sm:px-6 lg:px-8 bg-card/20 backdrop-blur-sm">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 max-w-6xl mx-auto">
          {plans.map((plan, index) => (
            <Card key={index} className={`relative h-full glass-card border-border/20 hover:border-primary/30 transition-all duration-300 ${plan.popular ? 'border-primary/50 shadow-lg shadow-primary/10' : ''}`}>
              {plan.popular && (
                <Badge className="absolute -top-2 left-1/2 transform -translate-x-1/2 bg-primary text-primary-foreground text-xs sm:text-sm">
                  {t('popular') || "Populaire"}
                </Badge>
              )}
                ...
                <ul className="space-y-2 sm:space-y-3">
                  {plan.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-start">
                      ...
                      <span className="text-xs sm:text-sm leading-relaxed text-foreground">{feature}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
              <CardFooter className="pt-4">
                <Button 
                  className={`w-full text-sm sm:text-base rounded-full ${plan.popular ? 'bg-primary hover:bg-primary/90 text-primary-foreground' : 'border-border/50 hover:border-primary/50'}`}
